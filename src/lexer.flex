...
%{
  ...
  std::unordered_map<std::string,std::string> typedf_map;
  void add_typedef(std::string identifier, std::string type){
		typedf_map[identifier] = type;
	}
...
{L}({L}|{D})*		{yylval.string = new std::string(yytext);
                  ...
                  if (typedf_map.count(name)){
                    std::string type = typedf_map[name];
                    if (type == "int"){
                      return(INT);
                    }
                    else {
